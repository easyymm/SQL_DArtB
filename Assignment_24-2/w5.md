...
DATETIME에서 특정 시간 표현 부분만 추출
```
SELECT
    EXTRACT(DATE FROM DATETIME "2024-10-29 19:04:15") AS date,
    EXTRACT(YEAR FROM DATETIME "2024-10-29 19:04:15") AS year,
    EXTRACT(MONTH FROM DATETIME "2024-10-29 19:04:15") AS month,
    EXTRACT(DAY FROM DATETIME "2024-10-29 19:04:15") AS day,
    EXTRACT(HOUR FROM DATETIME "2024-10-29 19:04:15") AS hour,
    EXTRACT(MINUTE FROM DATETIME "2024-10-29 19:04:15") AS minute,
    EXTRACT(DAYOFWEEK FROM DATETIME "2024-10-29 19:04:15") AS dow # 요일 추출(일요일 기준)
    ;
```
...
시간 자르기. 
```
예) DATETIME_TRUNC(datetime_col, HOUR)
 -> HOUR까지만 남기고 자름

SELECT
    DATETIME "2024-10-29 19:13:47" AS original_data,
    DATETIME_TRUNC(DATETIME "2024-10-29 19:13:47", DAY) AS day_trunc
    DATETIME_TRUNC(DATETIME "2024-10-29 19:13:47", YEAR) AS year_trunc
    DATETIME_TRUNC(DATETIME "2024-10-29 19:13:47", MONTH) AS month_trunc
    DATETIME_TRUNC(DATETIME "2024-10-29 19:13:47", HOUR) AS hour_trunc
```
...
```
SELECT
    PARSE_DATETIME('%Y-%m-%d %H:%M:%S', '2024-10-29 19:19:29') AS parse_datetime;
```
...
```
SELECT
    FORMAT_DATETIME("%c", DATETIME '2024-10-29 19:19:29') AS formatted;
```
...
```
SELECT
    LAST_DAY(DATETIME '2024-10-29 19:24:18') AS last_day,
    LAST_DAY(DATETIME '2024-10-29 19:24:18', MONTH) AS last_day_month,
    LAST_DAY(DATETIME '2024-10-29 19:24:18', WEEK) AS last_day_week,
    LAST_DAY(DATETIME '2024-10-29 19:24:18', WEEK(SUNDAY)) AS last_day_week_sun,
    LAST_DAY(DATETIME '2024-10-29 19:24:18', WEEK(MONDAY)) AS last_day_week_mon;
```
...
FROM(
    SELECT
        DATETIME "2024-10-29 19:32:11" AS first_datetime
        DATETMNE "2022-01-23 15:30:13": AS second_datetme,
    );
```
